...
Rhd2000EvalBoardUsb3::OpalKellyBoardType Rhd2000EvalBoardUsb3::open()
{
    ...
    cout << "---- Intan Technologies ---- Rhythm RHD2000 USB3 Controller v2.0 ----" << endl
         << endl;
    ...
    cout << endl
         << "Scanning USB for Opal Kelly devices..." << endl
         << endl;
    nDevices = dev->GetDeviceCount();
    cout << "Found " << nDevices << " Opal Kelly device" << ((nDevices == 1) ? "" : "s") << " connected:" << endl;
    for (i = 0; i < nDevices; ++i)
    {
        cout << "  Device #" << i + 1 << ": Opal Kelly " << opalKellyModelName (dev->GetDeviceListModel (i)).c_str() << " with serial number " << dev->GetDeviceListSerial (i).c_str() << endl;
    }
    cout << endl;
    ...
    cout << "Opal Kelly device firmware version: " << dev->GetDeviceMajorVersion() << "." << dev->GetDeviceMinorVersion() << endl;
    cout << "Opal Kelly device serial number: " << dev->GetSerialNumber().c_str() << endl;
    cout << "Opal Kelly device ID string: " << dev->GetDeviceID().c_str() << endl
         << endl;
...
bool Rhd2000EvalBoardUsb3::uploadFpgaBitfile (string filename)
{
    ...
    {
        cout << "Rhythm USB3 configuration file successfully loaded." << endl
             << endl;
    }
...
bool Rhd2000EvalBoardUsb3::readDataBlock (Rhd2000DataBlockUsb3* dataBlock, int nSamples)
{
    ...
    {
        cerr << "Error in Rhd2000EvalBoardUsb3::readDataBlock: USB buffer size exceeded.  " << "Increase value of MAX_NUM_BLOCKS." << endl;
        return false;
    }
...
bool Rhd2000EvalBoardUsb3::readDataBlocks (int numBlocks, queue<Rhd2000DataBlockUsb3>& dataQueue)
{
    ...
    if (numBytesToRead > usbBufferSize)
    {
        cerr << "Error in Rhd2000EvalBoardUsb3::readDataBlocks: USB buffer size exceeded.  " << "Increase value of MAX_NUM_BLOCKS." << endl;
        return false;
    }
...
void Rhd2000EvalBoardUsb3::startStim()
{
    ...
    {
        //cout << "outputting pattern file." << endl;
        //cout << "repeat count " << repeatCount << endl;
        //cout << "patternLength " << patternLength << endl;
        dev->SetWireInValue (StimPatternAddress, 0);
        dev->SetWireInValue (StimPatternRepeatCount, repeatCount);
        dev->SetWireInValue (StimPatternPageCount, (patternLength / 256));
        dev->UpdateWireIns();
        std::this_thread::sleep_for (std::chrono::milliseconds (10));

        //undefined behavior if length is 0. do not trigger. make this
        //populated by reading flash first to determine uploaded patterns
        if (patternLength > 0)
        {
            //cout << "outputting pattern file." << endl;
            //output
            dev->ActivateTriggerIn (START_STIM_TRIG, 0x1);
        }
        // else {
        //
        //cout << "non-valid pattern file on flash, aborting";
        //     }
    }
    ...
    {
        //cout << "outputting simple pulse" << endl;
        dev->ActivateTriggerIn (0x47, 0x0);
    }
}
...
{
    lock_guard<mutex> lockOk (okMutex);
    //activate both stop triggers
    dev->ActivateTriggerIn (0x47, 0x2);
}

void Rhd2000EvalBoardUsb3::setStimOutputType (int outputType)
{
    lock_guard<mutex> lockOk (okMutex);
    dev->SetWireInValue (StimOutputTypeSelect, outputType);
    dev->UpdateWireIns();
    selectedOutputType = outputType;
}

void Rhd2000EvalBoardUsb3::setPatternStimRepeatCount (int count)
{
    lock_guard<mutex> lockOk (okMutex);
    repeatCount = count;
}
...
void Rhd2000EvalBoardUsb3::uploadWaveformParameters (int waveform_addr, int edge_select, int amplitude, int period, int pulse_width, int pulse_count)
{
    ...
    dev->UpdateWireIns();
    dev->ActivateTriggerIn (PULSE_OUTPUT_PARAMS, 0x1); // Trigger update of waveform parameters
}
...
void Rhd2000EvalBoardUsb3::setStimChannel (int channel, int waveform_addr, int trig_mode, int trig_source, int enabled)
{
    ...
    //set stimulation of current channel.
    ...
    dev->UpdateWireIns();
    dev->ActivateTriggerIn (PULSE_OUTPUT_PARAMS, 0x2); // Trigger update of channel properties
}

void Rhd2000EvalBoardUsb3::setStimVoltage (bool stimVoltage)
{
    lock_guard<mutex> lockOk (okMutex);
    if (stimVoltage)
    {
        //set stimulation voltage high
        dev->SetWireInValue (WireInG3VoltageSet, 1, 0x01);
    }
    else
    {
        //set stimulation voltage low
        dev->SetWireInValue (WireInG3VoltageSet, 0, 0x01);
    }
    dev->UpdateWireIns();
    //give chip time to power up fully.
    std::this_thread::sleep_for (std::chrono::milliseconds (10));
}
...
void Rhd2000EvalBoardUsb3::trigEraseSectors (int address, int sectors)
{
    ...
    dev->SetWireInValue (LENGTH, sectors); // Send the number of sectors to erase
    dev->UpdateWireIns();
    dev->UpdateTriggerOuts();
    dev->ActivateTriggerIn (CMD_TRIG, 3);
...
//write data to flash starting at first sector.
int Rhd2000EvalBoardUsb3::trigWrite (const string& filename, int file_size)
{
    lock_guard<mutex> lockOk (okMutex);

    //safeguard in initialize board, sets patternLength to 0
    //upon start up to prevent undefined output behaviour.
    if (file_size < 0)
    {
        patternLength = 0;
        return -1;
    ...
    patternLength = file_size; //public variable for ease of use currently.
    //Defining the I/O
    std::ifstream bit_in;
    ...
    int sectorCount = (file_size + FLASH_SECTOR_SIZE - 1) / FLASH_SECTOR_SIZE;
    trigEraseSectors (0x0000, sectorCount);

    cout << "Sector Count: " << sectorCount << endl;

    //BEGIN WRITE OPERATION
    int num_chunks = (file_size + MAX_TRANSFER_SIZE - 1) / MAX_TRANSFER_SIZE;

    cout << "Num Chunks: " << num_chunks << endl;

    cout << "Writing Pattern File: " << filename << "..." << endl;
    ...
    for (int i = 0; i < num_chunks; i++)
    {
        //cout << "j:" << j << endl;
        fflush (stdout);
        int bytes_to_read = MIN (MAX_TRANSFER_SIZE, remaining_size);
        bit_in.read ((char*) buf, bytes_to_read); //reads a chunk of data from the buffer and stores it in bitfile

        //cout << "Writing: " << buf[0] << endl;

        int result = dev->WriteToPipeIn (DATA_IN_PIPE, MAX_TRANSFER_SIZE, buf); //sends max transfer size worth of data to FPGA
        //  cout << "Pipe In Write Result: " << result << endl;

        dev->SetWireInValue (ADDR, j); //sets starting address
        dev->SetWireInValue (LENGTH, (MAX_TRANSFER_SIZE / FLASH_PAGE_SIZE) - 1); //set number of pages being written (a 0 means 1 page to be written.)
        dev->UpdateWireIns();
        dev->UpdateTriggerOuts();
        dev->ActivateTriggerIn (CMD_TRIG, 5); //trigger programming
        ...
        {
            dev->UpdateTriggerOuts();
            if (dev->IsTriggered (FLASH_DONE, 0x0001))
                break;

            k++;
            if (k > 50)
            {
                printf ("\nError: Timeout in WritePatternfile -- Programming failed.\n");
                return -1;
            }
        } while (1);
    }
