...
public:
    ...
    int setSampleRate (int index, bool temporary = false);

  
   //setting cable delay
    void setCableDelay (int hsNum, int delay);
    ...
    void uploadStimParameters (int amplitude, int period, int pulse_width, int edge_sel, int pulse_count, int enableLEDS[64]);
    void startStim();
    void stopStim();
    void ledTestSequence (int led);
    int uploadPatternFile (File patternStimFile);
    void setPatternStimRepeatCount (int count);
    void setStimOutputType (int outputSelection);
