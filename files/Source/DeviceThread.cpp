...
bool DeviceThread::openBoard (String pathToLibrary)
{
    ...
    else // board could not be opened
    {
        bool response = AlertWindow::showOkCancelBox (AlertWindow::NoIcon,
                                                      "HectoSTAR Controller not found.",
                                                      "A HectoSTAR Controller could not be found. Please connect one now.",
                                                      "OK",
                                                      "Cancel",
                                                      0,
                                                      0);
...
void DeviceThread::initializeBoard()
{
    ...
    setSampleRate (Rhd2000EvalBoardUsb3::SampleRate20000Hz);

    evalBoard->setCableLengthMeters (Rhd2000EvalBoardUsb3::PortC, 4);
    ...
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortC, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    ...
    //GALVANI SPECIFIC
    enableAdcs (true); //raw TTL data is saved as ADC channel GALVANI

    //prevent unwanted patterns to output when stim is pressed.
...
void DeviceThread::scanPorts()
{
    ...
    setSampleRate (Rhd2000EvalBoardUsb3::SampleRate20000Hz, true);

    // Enable only G3 Data stream PortC MISO1 and MISO2 DDR (9 and 11)
    ...
    //enable both data streams.
    evalBoard->enableDataStream (9, true);
    evalBoard->enableDataStream (11, true);
    ...
    //Simply enable only G3 a
    int chipIdx = 0;
    hs = 4;

    LOGD ("Enabling G3")
    enableHeadstage (4, true, 2, 32);
    enableHeadstage (5, true, 2, 32);

    chipId.set (chipIdx++, CHIP_ID_GALVANI);

    updateBoardStreams();

    LOGD ("Number of enabled data streams: ", evalBoard->getNumEnabledDataStreams());

    setSampleRate (14); // set to 20Khz
                                   ...
                                   OwnedArray<ConfigurationObject>* configurationObjects)
{
    ...
    DataStream::Settings dataStreamSettings {
        "HectoSTAR Data",
        "128 Electrode and 64 Channel LED from Galvani3 Device",
        "hectostar_device.data ",
    ...
    EventChannel::Settings settings {
        ...
        "HectoSTAR FPGA TTL Input",
        "Events on digital input lines of a HectoSTAR device",
        "galvani-fpga-device.events",
        stream,
        64
    };
...
int DeviceThread::setSampleRate (int sampleRateIndex, bool isTemporary)
{
    impedanceThread->stopThreadSafely();
    if (! isTemporary)
    {
    ...
    //get resulting cable delay. G3 is considered to be on PortC
    int delay = evalBoard->getCableDelay (Rhd2000EvalBoardUsb3::PortC);
    LOGD ("Resulting G3 cable delay is: ", delay);
...
void DeviceThread::updateRegisters()
{
    ...
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortA, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortB, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortC, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortD, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortE, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortF, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortG, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
    evalBoard->selectAuxCommandBank (Rhd2000EvalBoardUsb3::PortH, Rhd2000EvalBoardUsb3::AuxCmd3, settings.fastSettleEnabled ? 2 : 1);
}
...
void DeviceThread::setCableDelay (int hsNum, int delay)
{
    // Set the MISO sampling delay, which is dependent on the sample rate.
...
bool DeviceThread::updateBuffer()
{
    ...
    for (int samp = 0; samp < nSamps; samp++)
    {
        ...
        for (int dataStream = 0; dataStream < numStreams; dataStream++)
        {
            ...
            for (int chan = 0; chan < nChans; chan++)
            {
                ...
                //galvani remapping. ChannelMapping depends on the probe being used.
                //thisSample[channelMapping[channel]] = float (*(uint16*) (bufferPtr + chanIndex) - 32768) * 0.195f; //here set G3 specific uV/bit
                thisSample[channel] = float(*(uint16*)(bufferPtr + chanIndex) - 32768) * 0.195f; //here set G3 specific uV/bit
        ...
        */
        ...
        //read all ADC channels, ADC1 - 4 are TTLs
        // ADC1=LED[15:0]
        // ADC2=LED[31:16]
        // ADC3=LED[47:32]
        //
        ...
        for (int adcChan = 0; adcChan < 8; ++adcChan)
        {
            ...
            thisSample[channel] = 0.0003125 * float (*(uint16*) (bufferPtr + index));
        ...
        uint64 ttlEventWord = 0;
        // Put input TTL bits [0..6] into ttlEventWord bits [0..6]
        ttlEventWord |= (uint64) (inputTTLWord & 0x7F); // 0x7F = 0b01111111
    ...
    {
        LOGD ("UPDATE DURING ACQUISTION");
        //REMOVED FOR G3, THERE IS NO SETTINGS WE CAN UPDATE DURING ACQUISITION
        //STIMULATION HANDLED BY THE VISUALIZER.
        updateSettingsDuringAcquisition = false;
    }
...
//GALVANI 3
void DeviceThread::uploadStimParameters (int amplitude, int period, int pulse_width, int edge_sel, int pulse_count, int enabledLEDS[64])
{
    LOGD ("UPLOADING STIM PARAMETERS")
    LOGD ("WAVEFORM 0")
    LOGD ("amplitude: ", amplitude)
    LOGD ("period: ", period)
    LOGD ("pulse_width: ", pulse_width)
    LOGD ("edge_sel: ", edge_sel)
    LOGD ("pulse_count: ", pulse_count)

    //Need to scale the values given depending on the current sample rate of the system
    int scale = (evalBoard->getSampleRate() / 1000);
    //period = period * scale;
    //pulse_width = pulse_width * scale; remove??

    evalBoard->uploadWaveformParameters (0, edge_sel, amplitude, period * scale, pulse_width * scale, pulse_count);

    /*********************************************/
    //setStimChannel(int channel, int waveform_addr, int trig_mode (0 or 1), int trig_source (0 or 1), int enabled (0 or 1) {
    //need to set whether the current LED is enabled, which waveform it should output

    for (int i = 0; i < 64; i++)
    {
        evalBoard->setStimChannel (i, 0, 0, 0, enabledLEDS[i]);
    }
}

void DeviceThread::startStim()
{
    LOGD ("STARTING STIM")
    evalBoard->startStim();
}

void DeviceThread::stopStim()
{
    //stop stim is unified
    LOGD ("STOPPING STIM")
    evalBoard->stopStim();
}

void DeviceThread::setStimOutputType (int outputSelection)
{
    LOGD ("SETTING OUTPUT TO PULSE OR PATTERN")
    if (! outputSelection)
    {
        LOGD ("SET TO PULSE OUTPUT")
    }
    else
    {
        LOGD ("SET TO PATTERN OUTPUT")
    }
    evalBoard->setStimOutputType (outputSelection);
}

int DeviceThread::uploadPatternFile (File patternStimFile)
{
    LOGD ("UPLOADING PATTERN FILE")
    std::string filePath = patternStimFile.getFullPathName().toStdString();
    int file_size = patternStimFile.getSize();
    return evalBoard->trigWrite (filePath, file_size);
}

void DeviceThread::setPatternStimRepeatCount (int count)
{
    evalBoard->setPatternStimRepeatCount (count);
}
